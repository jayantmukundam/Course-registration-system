...
const CourseForm = ({ userData }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async ()=>{
            const response = await fetch(`http://localhost:8080/api/courseSchedule/${Number(userData.employee_id)}/get`);
    ...
    const handleAddEmployee = async (index)=>{
            ...
            let course_id = courseList[index].course_id.course_id
                ...
                const response = await fetch(`http://localhost:8080/api/course/${Number(userData.employee_id)}/add/${Number(course_id)}`);
                const newData = await response.json();
                console.log(newData);
                setCourseList(newData);
